...
public class GameManager : MonoBehaviour {
	...
	void Start () 
	{
        ...
        this.targetScore = GameObject.Find("TargetScore").GetComponent<Text>();
        this.passMessage = GameObject.Find("PassMessage").GetComponent<Text>();
	...
	void Update () 
	{
		...
		else if (this.state == State.PLAY) 
		{
			...
			{
                if (ScoreManager.score >= Mathf.Exp(level-1) * 130)
                {
                    this.state = State.PASS;
                    this.anim.SetTrigger("PassTrigger");
                    passMessage.text = "Congratulation!You have passed level " + level;
            ...
            if (Input.GetMouseButtonDown(1))
            {
                ...
                this.timer = 0;
                level += 1;
                this.moleManager.proSpeed -= 0.2f;
                this.moleManager.StartGenerate();
            }
        }
